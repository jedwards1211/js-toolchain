...
[![npm version](https://badge.fury.io/js/%40jedwards1211%2Fts-toolchain.svg)](https://badge.fury.io/js/%40jedwards1211%2Fts-toolchain)
...
- Test files are in `test`. You can override this by specifying options for mocha in `['@jedwards1211/ts-toolchain'].mochaArgs` in your `package.json`.
...
```sh
yarn add --dev @jedwards1211/ts-toolchain
...
The bootstrap script will modify your package.json, .gitignore, .circleci/config.yml and generate .babelrc.js, .eslintrc.js, and .prettierrc.js that are just proxies for the config in `@jedwards1211/ts-toolchain`.
