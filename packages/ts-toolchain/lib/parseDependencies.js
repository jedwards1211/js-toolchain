...
module.exports = async function parseDependencies(dist) {
  const dependencies = new Set()

  function processSource(source) {
    if (
      !source.node ||
      source.node.type !== 'StringLiteral' ||
      source.node.value.startsWith('.')
    )
      return

    const match = /^(@[^/]+\/)?[^/]+/.exec(source.node.value)
    if (match) dependencies.add(match[0])
  }
